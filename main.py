...
class GCP:
    ...
    def __init__(self, wavelength, n_cladding, n_core, n_substrate, core_height):
        ...
        GCP.coreSlabTEMmodes(self, 100,1)
    ...
    def coreSlabTEMmodes(self, no_points,plot_slab):
        ...
        self.y2 = (self.gamma_cladding + self.gamma_substrate) / (self.kappa * (1 - (self.gamma_cladding * self.gamma_substrate / self.kappa ** 2)))
        self.y3 = self.kappa*((self.n_core**2/self.n_substrate**2) * self.gamma_substrate + (self.n_core**2/self.n_cladding**2)*self.gamma_cladding) / (self.kappa**2 - (self.n_core**4 / ((self.n_cladding ** 2) * (self.n_substrate**2))*self.gamma_cladding * self.gamma_substrate))
    ...
    def BraggPhaseMatch(m, k_0, n_cladding, theta, n_eff):
        ...
        BPM_eqn = ((k_0 * n_cladding * m.sin(theta) + m * 2 * m.pi / grating_period) / k_0) - n_eff
